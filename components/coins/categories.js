...
const Categories = ({ data, navigationItemData, inputSearch, handleSelect }) => {
  ...
  return (
    <div className="max-h-96 overflow-y-scroll">
      {categoriesData && categoriesData.map((item, i) => (
        <div key={i}>
          <Link href={`/coins/${item && item.category_id ? item.category_id : 'categories'}`}>
            <a
              onClick={handleSelect}
              className={`w-full flex items-center justify-start ${item && item.category_id === coin_type ? 'bg-indigo-600 text-white' : 'bg-transparent hover:bg-indigo-50 text-indigo-500 hover:text-indigo-600 dark:hover:bg-indigo-900 dark:text-white dark:hover:text-gray-200'} text-xs space-x-2 p-2`}
            >
              {item.image && (
                <Image
                  src={item.image}
