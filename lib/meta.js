...
export default function getMeta(path, data) {
  ...
  if (pathSplit[0] === 'coins') {
    if (pathSplit[1] === 'categories') {
      ...
      title = `Top Decentralized Finance (DeFi) Coins by Market Cap | ${process.env.NEXT_PUBLIC_APP_NAME}`
    ...
    title = `Today's Latest Cryptocurrency News | ${process.env.NEXT_PUBLIC_APP_NAME}`
