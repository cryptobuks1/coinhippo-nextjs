...
export const weightedRand = values => {
  values = typeof values === 'object' ? values : Array.isArray(values) ? values : [values]
  const valuesWeight = typeof values === 'object' ? values : { ...(values.map((value, i) => .5 * (1 + (values.length - (i + 1))))) }

  let i, sum = 0, rand = Math.random()
  for (i in valuesWeight) {
    sum += valuesWeight[i]
    if (rand <= sum) return typeof values === 'object' ? i : values[i]
  }
}